...
void cBowComponent::GameDone(unsigned int totalBalloonsSpawned)
{
    ...
    std::cout << "\tArrows fired: " << this->projectilesFiredCount << std::endl;
    std::cout << "\tYou missed " << totalBalloonsSpawned - this->balloonsPopped << " balloons!" << std::endl << std::endl;
}
...
void cBowComponent::RemoveProjectile(cEntity* proj)
{
    for (size_t i = 0; i < this->projectilesFired.size(); i++)
    {
        if (this->projectilesFired[i]->GetEntity() == proj)
        {
...
void cBowComponent::FireProjectile(glm::vec3 pos, glm::vec3 direction, float aimingValue)
{
    ...
    cProjectile* proj = new cProjectile(this, trans, direction, 65.0f * aimingValue + 10.0f);
